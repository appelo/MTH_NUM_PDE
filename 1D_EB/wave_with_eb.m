...
% DEAA Find local coordinates where the boundary is
% Use bisection / secant method
...
for it = 1:nsteps
    ...
    % DEAA Apply the mask to the right hand side.  
    rhs = (mask==1).*(uxx+f);
    up = 2*u-um + k^2*rhs;
...
% DEAA REPRESENTATION OF THE BOUNDARY
function f = is_inside(x) 
...
% This function returns u at t on the boundary x = 0
    u = exp(-(t-10)^2)*(5*sin(t)-pi*cos(t));
end

function u = h_1(t)
% This function returns u at t on the boundary x = L
    u = exp(-(t-10)^2)*(4*sin(t-pi/3)-2*pi*cos(t-pi/3));
...
% to the wave equation
    f = exp(-(t-10)^2)*sin(10*t)*exp(-36*(x-1/2).^2);
end
